...
async function fetchData() {
  console.log("🔄 [Device] fetchData called - Starting to fetch device data");
  console.log("📊 [Device] Current queryParams:", queryParams.value);

  try {
    loading.value = true;
    console.log("⏳ [Device] Loading state set to true");

    const data = await DeviceAPI.listDevices(queryParams.value);
    console.log("✅ [Device] API call successful, received data:", data);

    pageData.value = data;
    console.log("📋 [Device] Page data updated, total devices:", data?.length || 0);
  } catch (error) {
    console.error("❌ [Device] Error fetching devices:", error);
    ElMessage.error(t("device.fetchError"));
  } finally {
    loading.value = false;
    console.log("🏁 [Device] Loading state set to false - fetchData completed");
  }
}
 ...
 */
onMounted(() => {
  console.log("🚀 [Device] Component mounted - Initializing device management");
  console.log("📱 [Device] Current device type:", appStore.device);
  console.log("🌐 [Device] Current language:", t("device.title"));

  fetchData();

  console.log("✅ [Device] Component initialization completed");
});
</script>
